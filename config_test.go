...
func Test_Config_copy(t *testing.T) {
	middleware := func(context.Context, *kafka.Message) error { return nil }
	...
	t.Run("copies middleware", func(t *testing.T) {
		copy := cfg.copy()
...
func Test_Config_WithMiddleware(t *testing.T) {
	middleware := func(ctx context.Context, msg *kafka.Message) error { return nil }
...
func Test_Config_WithNoClient(t *testing.T) {
	...
	t.Run("sets HasNoBroker", func(t *testing.T) {
		wanted := true
		got := copy.HasNoBroker()
		if wanted != got {
			t.Errorf("wanted %v, got %v", wanted, got)
		}
	})
